    ...
    create or replace table `manuel-demo-1392926998`.`analytics`.`dim_customer_cohort`
    ...
    OPTIONS(
      description="""One row per customer capturing cohort_date (first ever active) and cohort_month (bucketed), plus stable slicing attrs.\n"""
    )
    as (
      -- ========================================================================================
...
left join `manuel-demo-1392926998`.`analytics`.`dim_acquisition_taxonomy` tc using (customer_id)
    );
