...
{{ config(
  ...
  tags=['marts','acquisition','churn','monthly','movements']
) }}

{% set lookback_days = var('movements_lookback_days', var('test_lookback_days', 365)) %}
...
churned_by_month as (
  ...
  from
	...
	from {{ ref('fct_customer_active_monthly') }}
	group by 1)
  group by 1
),
